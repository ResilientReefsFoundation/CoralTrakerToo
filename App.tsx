...
const App: React.FC = () => {
  ...
  const LOCAL_STORAGE_KEY = 'coral-nursery-app-v1';
  ...
  const clearAllData = React.useCallback(() => {
      ...
      setSpeciesList([]); // Also clear species list when wiping
  }, []);

  const loadAllDataFromBackup = (data: any, isCloud: boolean = false) => {
      setCoralBranches(data.coralBranches || []);
      setActiveBranchId(data.coralBranches?.[0]?.id || '');
      setRules(data.rules || []);
      setSites(data.sites || []);
      setZones(data.zones || []);
      ...
      setTempLoggers(data.tempLoggers || []);
      setMaintenanceLogs(data.maintenanceLogs || []);
      setToDoItems(data.toDoItems || []);
      // Voice notes are not synced to cloud/local storage easily due to blob URLs, but if stored as Base64 they could be.
      // For now, we assume localStorage treated as "cloud" for this logic
      if (!isCloud) setVoiceNotes(data.voiceNotes || []);
      // Schedule is ephemeral
      setSchedule(new Map());
      setTreeShadeExperiment(data.treeShadeExperiment || null);
      setRopeOnRubbleExperiment(data.ropeOnRubbleExperiment || null);
      setSquareRopeFrameExperiment(data.squareRopeFrameExperiment || null);
      setCubeRopeFrameExperiment(data.cubeRopeFrameExperiment || null);
  ...
  const allData = React.useMemo(() => ({
    coralBranches, rules, speciesList, sites, zones, anchors, trees, floats, activityLog, tempLoggers, maintenanceLogs, toDoItems,
    treeShadeExperiment, ropeOnRubbleExperiment, squareRopeFrameExperiment, cubeRopeFrameExperiment
    // Note: voiceNotes and schedule are not saved
  }), [coralBranches, rules, speciesList, sites, zones, anchors, trees, floats, activityLog, tempLoggers, maintenanceLogs, toDoItems, treeShadeExperiment, ropeOnRubbleExperiment, squareRopeFrameExperiment, cubeRopeFrameExperiment]);
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        if (localData) {
          ...
          if (parsed && (parsed.coralBranches || parsed.sites)) {
            loadAllDataFromBackup(parsed, true);
            setSyncStatus('saved');
          } else {
            loadSampleData(); // Storage is empty or invalid, seed with sample data
          }
        } else {
          // No local storage found. Seed with sample data.
          loadSampleData();
        }
      } catch (error) {
        ...
        // Use a ref to gate the auto-save effect until after this initial load is complete.
  ...
  React.useEffect(() => {
    if (!dataInitialized.current) {
        // This prevents the effect from running on the initial render before data is loaded.
        return;
    }
    ...
    const handler = setTimeout(() => {
      try {
        ...
        if (error instanceof DOMException && error.name === 'QuotaExceededError') {
             alert("Storage full! Cannot save new data. Please delete some items or photos.");
        }
        setSyncStatus('error');
      }
  ...
  const handleWipeAllData = () => {
    if (window.confirm('This will wipe all local data (including any cloud-synced data stored locally) and cannot be undone. Are you sure?')) {
      clearAllData();
      logActivity('All local nursery data has been wiped.', 'general');
    }
  };
  ...
  const handleRestoreNurseryData = (backup: any) => {
    try {
        ...
        loadAllDataFromBackup(backup, false);
        setSyncStatus('saving'); // Trigger cloud/local save of restored data.
        
        logActivity('Nursery data restored from backup file.', 'general');
        alert('Nursery data restored successfully! It will now be synced.');
    } catch(error) {
  ...
  const handleRestoreSpeciesIdData = (backup: any) => {
      try {
          ...
          setSyncStatus('saving'); // Trigger cloud save
          
          logActivity('Species ID data restored from backup file.', 'general');
          alert('Species ID data restored successfully! It will now be synced.');
      } catch(error) {
  ...
  const handleResetToSampleData = () => {
    if (window.confirm('This will overwrite ALL data with the initial sample data. This cannot be undone. Are you sure?')) {
      loadSampleData();
    }
  };
  ...
  const handleRemoveTempLogger = (loggerId: string) => {
    ...
    const handleAddTreeShadeReport = (notes: string) => {
        ...
        const getTreeStats = (treeNumber: number) => {
            // FIX: "branches" is not defined. Using "activeBranches" which is available in this scope and contains the relevant data.
