...
const BackupRestorePage: React.FC<BackupRestorePageProps> = ({ 
  ...
  const handleRestoreFileChange = (e: ChangeEvent<HTMLInputElement>, restoreType: 'nursery' | 'speciesId') => {
    ...
    if (file) {
      const confirmationMessage = restoreType === 'nursery'
        ? 'Are you sure you want to restore nursery data from this file? This will overwrite all current cloud data.'
        : 'Are you sure you want to restore Species ID data from this file? This will overwrite your current cloud Species ID photos and notes.';
  ...
  const handleWipeData = () => {
    if (deleteConfirmText === 'DELETE') {
      if (window.confirm('Are you absolutely sure you want to wipe all local (non-cloud) data? This action is irreversible.')) {
        onWipeAllData();
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg space-y-8">
      ...
      <div className="p-4 border rounded-lg space-y-4">
        ...
        <p className="text-sm text-gray-500">Save a copy of the current cloud data to a file on this device, or restore the cloud data from a previously saved file.</p>
      ...
      {/* App Data Management */}
      <div className="p-4 border rounded-lg space-y-4">
        <h3 className="font-semibold text-gray-700 text-lg">App Data Management</h3>
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-500 max-w-prose">
            Reset all cloud data to the initial sample data set. This will overwrite any current data for all users.
          </p>
          <button
      ...
      <div className="p-4 border border-red-500 rounded-lg space-y-4">
        ...
        <div>
          <label htmlFor="deleteConfirm" className="block text-sm font-medium text-gray-700">
            To wipe all non-cloud data (e.g., voice notes), type "DELETE" below and click the button. This action is irreversible.
          </label>
          <input
            type="text"
            id="deleteConfirm"
